...
class CMakeiOSToolchain(object):
    ...
    _template_toolchain = textwrap.dedent("""
        ...
        set(CMAKE_OSX_ARCHITECTURES "{{ generator_platform }}" CACHE STRING "" FORCE)
        {%- endif %}

        set(CMAKE_OSX_DEPLOYMENT_TARGET "{{ os_version }}" CACHE STRING "Set CMake deployment target" ${FORCE_CACHE})

        set(XCODE_VERSION "{{ compiler_version }}")
    ...
    def __init__(self, conanfile, generator=None, generator_platform=None, build_type=None,
        ...
        self.generator = generator or "XCode"
        self.generator_platform = generator_platform or self.get_generator_platform(
        ...
        self.build_type = build_type if not self.generator == "Xcode" else None
        try:
    ...
    def get_generator_platform(self):
