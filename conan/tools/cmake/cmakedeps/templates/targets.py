...
class TargetsTemplate(CMakeDepsFileTemplate):
    ...
    def context(self):
        ...
        target = "%s" % self.global_target_name
        ...
        for comp_name in self.conanfile.cpp_info.components:
            ...
            target = "%s" % self.get_component_alias(self.conanfile, comp_name)
    ...
    def template(self):
        ...
        foreach(_COMPONENT {{ '${' + pkg_name + '_COMPONENT_NAMES' + '}' }} )
            if(NOT TARGET {{ pkg_name }}::${_COMPONENT})
                ...
                conan_message(STATUS "Conan: Component target declared '{{ pkg_name }}::${_COMPONENT}'")
            endif()
        endforeach()
