...
BUILT_IN_CONFS = {
    ...
    "tools.system.condaenv:micromamba_path": "(Experimental) Path to a micromamba executable to be used by CondaEnv. If unset, CondaEnv looks in CONAN_CONDAENV_MICROMAMBA and then in PATH",
    "tools.system.condaenv:root_prefix": "(Experimental) Root directory for the conda package cache shared across recipes",
    "tools.system.condaenv:default_channels": "(Experimental) Default list of conda channels used by CondaEnv when none are passed",
    "tools.system.condaenv:conda_pack_path": "(Experimental) Path to an existing conda-pack executable used by CondaEnv.pack()",
